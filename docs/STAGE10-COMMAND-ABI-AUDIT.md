...
`sakla` için ABI/capability çekirdeği, parser/AST → mutation-ABI shape binding, authoritative semantic metadata, backend host-mutation doğrulaması, transaction-backed host write, kalıcı SQLite adapterı ve native↔WASM kalıcı-adapter equivalence regression kapısı vardır. Sıradaki gerçek eksik dilim kaynak runtime'daki `sakla <Name>` davranışını authoritative semantic binding ve mevcut transaction-backed mutation execution zincirine bağlamak; ardından bu uçtan uca referans runtime gözlemini native/WASM sonuçlarıyla karşılaştırmaktır. Bu kapı kapanmadan `sakla` desteklenmiş sayılmaz.
