...
- Gerçek `.shn` kaynak equivalence testi mutation değerini referans runtime'dan kopyalayıp backend sonucuymuş gibi kullanmaz; native ve WASM için ilgili backend gözlemindeki exact slot okunur ve referans slot yalnız beklenen sonuç olarak kullanılır.
...
`sakla` için ABI/capability çekirdeği, parser/AST → mutation-ABI shape binding, authoritative semantic metadata, backend host-mutation doğrulaması, transaction-backed host write, kalıcı SQLite adapterı, native↔WASM kalıcı-adapter equivalence regression kapısı, kaynak runtime slotunu bu zincire bağlayan fail-closed execution köprüsü ve gerçek `.shn` kaynak üzerinden referans/native/WASM slot→persistent-mutation equivalence kapısı vardır. Sıradaki gerçek eksik dilim README'deki `kayıt Ürün` / `ürün` örneğine denk gelen kayıt/declaration değerinin authoritative model metadata ile gerçek production SQLite yazımına kaynak-program düzeyinde bağlanmasıdır. Bu kapı kapanmadan `sakla` ve diğer host etkili Command kapsamı tamamlanmış sayılmaz.
