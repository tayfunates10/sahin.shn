...
Aşama 10 henüz tamamlanmış değildir. `TryStatement` control-flow/error-binding/backend handler çekirdeği, top-level ve flow-local `Binary` hata source-provenance/payload eşdeğerliği ile flow dışına taşan hata için call-site provenance/stack-frame ABI'sı doğrulandı; diğer hata üretebilen opcode'ların gözlemlenebilir payload ABI'ları açık kalmaktadır. Bunun ardından `ExpressionStatement`, `FieldDeclaration`, diğer `Command` ve diğer `Declaration` düğümleri kendi semantik/ABI/capability modelleriyle kapatılacaktır.
