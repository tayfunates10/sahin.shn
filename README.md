...
Aşama 10 henüz tamamlanmış değildir. `TryStatement` control-flow/error-binding/backend handler çekirdeği, top-level `Binary` hata source-provenance/payload eşdeğerliği ve flow `Binary` provenance taşıması doğrulandı; flow içindeki yakalanan hata payload yürütme/equivalence ile diğer hata üretebilen opcode'ların gözlemlenebilir payload ABI'ları açık kalmaktadır. Bunun ardından `ExpressionStatement`, `FieldDeclaration`, diğer `Command` ve diğer `Declaration` düğümleri kendi semantik/ABI/capability modelleriyle kapatılacaktır.
