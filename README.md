...
Aşama 10'da deterministik IR çekirdeği, güvenli WASM/native adapter sınırları, control-flow primitive'leri, gerçek `IfStatement` lowering + lexical scope korunumu, lazy RHS koruyan kısa devreli `ve/veya` lowering, referans runtime ile düz-akış backend planları arasındaki semantik eşdeğerlik oracle'ı, tekrarlanabilir benchmark baseline'ı ve property/security regression kapıları `main` üzerinde doğrulandı. Kalan ana iş control-flow semantik eşdeğerlik genişletmesi ve diğer eksik AST/semantic lowering kapsamıdır; final Python 3.11/3.12/3.13 + gerçek `.shn` smoke kapısı tamamlanmadan ilerleme %94'e çıkarılmaz.
...
Aşama 10 henüz tamamlanmış değildir. Control-flow semantik eşdeğerlik kapsamı ve diğer eksik AST/semantic düğümlerinin doğru control-flow/ABI modeliyle IR'a indirilmesi gerekir.
