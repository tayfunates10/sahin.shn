...
class Parser:
    ...
    def parse(self) -> Program:
        ...
        while not self._at(TokenKind.EOF):
            ...
            if self._at(TokenKind.INDENT):
                raise self._error(self._current(), "bu satırdan önce blok açan bir ifade bekleniyordu.")
    ...
    def _statement(self):
        ...
        if self._at(TokenKind.ASSIGN) or self._at(TokenKind.BIND):
            ...
            expression = self._finish_expression_line(self._expression())
            if operator.kind is TokenKind.BIND:
                return Binding(first.value, expression, self._loc(first))
            return Assignment(first.value, expression, location=self._loc(first))
    ...
    def _declaration(self, keyword: Token) -> Declaration:
        ...
        if kind in {"uygulama", "ekran", "görünüm", "kayıt"}:
            name = self._expect(TokenKind.IDENTIFIER, f"{kind!r} sonrasında bir ad bekleniyordu.").value
        elif kind == "akış":
            name = self._expect(TokenKind.IDENTIFIER, "'akış' sonrasında bir akış adı bekleniyordu.").value
            parameters = self._flow_parameters()
            if self._match(TokenKind.FAT_ARROW):
                inline = self._expression()
                self._line_end()
                return Declaration(kind=kind, name=name, parameters=tuple(parameters), inline_expression=inline, location=self._loc(keyword))
            if self._match(TokenKind.ARROW):
                return_type = self._expect(TokenKind.IDENTIFIER, "'->' sonrasında dönüş tipi bekleniyordu.").value
        elif kind == "uç":
            name = self._expect(TokenKind.IDENTIFIER, "'uç' sonrasında yöntem adı bekleniyordu.").value
            header = self._loose_header()
        elif kind == "iş":
            name = self._expect(TokenKind.IDENTIFIER, "'iş' sonrasında iş adı bekleniyordu.").value
            header = self._loose_header()
        elif kind == "olay":
            header = self._loose_header()
            if header:
        ...
        return Declaration(kind=kind, name=name, parameters=tuple(parameters), header=tuple(header), return_type=return_type, body=body, location=self._loc(keyword))

    def _flow_parameters(self) -> list[Parameter]:
        result: list[Parameter] = []
        while not self._at_any(TokenKind.NEWLINE, TokenKind.ARROW, TokenKind.FAT_ARROW, TokenKind.EOF):
            if self._match(TokenKind.COMMA):
                continue
            token = self._expect(TokenKind.IDENTIFIER, "Akış parametresi için bir ad bekleniyordu.")
            type_name = None
            if self._match(TokenKind.COLON):
                type_name = self._expect(TokenKind.IDENTIFIER, "':' sonrasında parametre tipi bekleniyordu.").value
    ...
    def _loose_header(self) -> list:
        ...
        while not self._at_any(TokenKind.NEWLINE, TokenKind.EOF):
            if self._match(TokenKind.COMMA):
                continue
            values.append(self._expression(allow_pipeline=False, allow_implicit_call=False, stop_kinds={TokenKind.COMMA, TokenKind.NEWLINE}))
        return values

    def _field_declaration(self, first: Token) -> FieldDeclaration:
        type_token = self._expect(TokenKind.IDENTIFIER, "Alan için bir tip adı bekleniyordu.")
        modifiers: list[str] = []
        while self._at(TokenKind.IDENTIFIER):
            modifiers.append(self._advance().value)
        self._line_end()
        return FieldDeclaration(first.value, type_token.value, tuple(modifiers), self._loc(first))

    def _if_statement(self) -> IfStatement:
        start = self._current()
        condition = self._expression(stop_words={"ise", "yok", "boş", "değil"}, allow_implicit_call=True)
    ...
    def _for_each(self, first: Token) -> ForEach:
        variable = self._expect(TokenKind.IDENTIFIER, "'her' sonrasında yineleme adı bekleniyordu.")
    ...
    def _match_statement(self, first: Token) -> MatchStatement:
        ...
        self._line_end()
        self._expect(TokenKind.INDENT, "'duruma göre' sonrasında girintili eşleşme bloğu bekleniyordu.")
        cases: list[MatchCase] = []
        while not self._at_any(TokenKind.DEDENT, TokenKind.EOF):
            if self._match(TokenKind.NEWLINE):
                continue
            pattern_start = self._current()
            pattern = self._expression(allow_pipeline=False, allow_implicit_call=False, stop_kinds={TokenKind.ARROW})
            self._expect(TokenKind.ARROW, "Eşleşme kolunda '->' bekleniyordu.")
            command_first = self._expect(TokenKind.IDENTIFIER, "'->' sonrasında çalıştırılacak komut bekleniyordu.")
    ...
    def _try_statement(self, first: Token) -> TryStatement:
        body = self._required_block()
        self._expect_keyword("olmazsa", "'dene' bloğundan sonra 'olmazsa' bekleniyordu.")
    ...
    def _command(self, first: Token, *, allow_block: bool = True) -> Command:
        ...
        if self._at(TokenKind.DOT):
            ...
            verb = self._expect(TokenKind.IDENTIFIER, "Nesne ifadesinden sonra uygulanacak Şahin eylemi bekleniyordu.")
            name = verb.value

        arguments = []
        while not self._at_any(TokenKind.NEWLINE, TokenKind.EOF, TokenKind.ARROW, TokenKind.DEDENT):
            if self._match(TokenKind.COMMA):
                continue
            arguments.append(self._expression(allow_pipeline=True, allow_implicit_call=False, stop_kinds={TokenKind.COMMA, TokenKind.NEWLINE, TokenKind.ARROW, TokenKind.DEDENT}))
        ...
        return Command(name=name, arguments=tuple(arguments), subject=subject, arrow=arrow, body=body, location=self._loc(first))

    def _expression(self, min_precedence: int = 0, *, allow_pipeline: bool = True, allow_implicit_call: bool = True, stop_words: set[str] | None = None, stop_kinds: set[TokenKind] | None = None):
        stop_words = stop_words or set()
        stop_kinds = stop_kinds or set()
        left = self._unary(allow_implicit_call=allow_implicit_call, stop_words=stop_words, stop_kinds=stop_kinds)
        ...
        while True:
            ...
            if operator is not None and precedence is not None:
                ...
                right = self._expression(precedence + 1, allow_pipeline=False, allow_implicit_call=allow_implicit_call, stop_words=stop_words, stop_kinds=stop_kinds)
                left = RangeExpression(left, right, self._loc(op_token)) if operator == ".." else Binary(left, operator, right, self._loc(op_token))
                continue
    ...
    def _unary(self, *, allow_implicit_call: bool, stop_words: set[str], stop_kinds: set[TokenKind]):
        if self._at(TokenKind.OPERATOR) and self._current().value in {"-", "+", "!"}:
            token = self._advance()
            return Unary(token.value, self._unary(allow_implicit_call=allow_implicit_call, stop_words=stop_words, stop_kinds=stop_kinds), self._loc(token))
        if self._match_keyword("değil"):
            token = self.tokens[self.index - 1]
            return Unary("değil", self._unary(allow_implicit_call=allow_implicit_call, stop_words=stop_words, stop_kinds=stop_kinds), self._loc(token))
        return self._postfix(allow_implicit_call=allow_implicit_call, stop_words=stop_words, stop_kinds=stop_kinds)

    def _postfix(self, *, allow_implicit_call: bool, stop_words: set[str], stop_kinds: set[TokenKind]):
        expression = self._primary(stop_words=stop_words, stop_kinds=stop_kinds)
        while True:
            if self._match(TokenKind.DOT):
                member = self._expect(TokenKind.IDENTIFIER, "'.' sonrasında üye adı bekleniyordu.")
            ...
            if self._match(TokenKind.LPAREN):
                ...
                while not self._at_any(TokenKind.RPAREN, TokenKind.EOF):
                    if self._match(TokenKind.COMMA):
                        continue
                    arguments.append(self._expression(stop_kinds={TokenKind.COMMA, TokenKind.RPAREN}))
                self._expect(TokenKind.RPAREN, "Çağrı için ')' bekleniyordu.")
                expression = Call(expression, tuple(arguments), self._loc_from_expression(expression))
                continue
            if allow_implicit_call and self._can_start_primary() and not (self._at(TokenKind.IDENTIFIER) and self._current().value in (_IMPLICIT_CALL_STOP | stop_words)):
                argument = self._postfix(allow_implicit_call=False, stop_words=stop_words, stop_kinds=stop_kinds)
                if isinstance(expression, Call):
                    expression = Call(expression.callee, (*expression.arguments, argument), expression.location)
                else:
                    expression = Call(expression, (argument,), self._loc_from_expression(expression))
                continue
            break
        return expression
    ...
    def _pipeline_stage(self) -> PipelineStage:
        name = self._expect(TokenKind.IDENTIFIER, "'|' sonrasında veri hattı aşaması bekleniyordu.")
        arguments = []
        while not self._at_any(TokenKind.PIPE, TokenKind.NEWLINE, TokenKind.DEDENT, TokenKind.EOF):
            if self._match(TokenKind.COMMA):
                continue
            arguments.append(self._expression(allow_pipeline=False, allow_implicit_call=False, stop_kinds={TokenKind.COMMA, TokenKind.PIPE, TokenKind.NEWLINE, TokenKind.DEDENT}))
    ...
    def _finish_expression_line(self, expression):
        ...
        if self._at(TokenKind.INDENT) and self._peek_kind(1) is TokenKind.PIPE:
            ...
            while self._at(TokenKind.PIPE):
                ...
                self._line_end()
            self._expect(TokenKind.DEDENT, "Çok satırlı veri hattı girintisi düzgün kapanmadı.")
            return Pipeline(source, tuple(stages), self._loc_from_expression(source))
        return expression

    def _required_block(self) -> tuple:
        self._line_end()
        self._expect(TokenKind.INDENT, "Bu ifade için 4 boşluk girintili bir blok bekleniyordu.")
    ...
    def _can_start_primary(self) -> bool:
        return self._current().kind in {TokenKind.IDENTIFIER, TokenKind.NUMBER, TokenKind.STRING, TokenKind.LPAREN}
