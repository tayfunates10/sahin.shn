...
def _unary_sites_expression(expression, sites: list[tuple[str, int, int]]) -> None:
    if isinstance(expression, Unary):
        _unary_sites_expression(expression.operand, sites)
        if expression.operator in {"+", "-"} and expression.location is not None:
            sites.append((expression.operator, expression.location.line, expression.location.column))
        return
    if isinstance(expression, Binary):
        _unary_sites_expression(expression.left, sites)
        _unary_sites_expression(expression.right, sites)
        return
    if isinstance(expression, Predicate):
        _unary_sites_expression(expression.expression, sites)
        return
    if isinstance(expression, Member):
        _unary_sites_expression(expression.target, sites)
        return
    if isinstance(expression, Call):
        _unary_sites_expression(expression.callee, sites)
        for argument in expression.arguments:
            _unary_sites_expression(argument, sites)
        return
    if isinstance(expression, RangeExpression):
        _unary_sites_expression(expression.start, sites)
        _unary_sites_expression(expression.end, sites)
        return
    if isinstance(expression, Pipeline):
        _unary_sites_expression(expression.source, sites)
        for stage in expression.stages:
            for argument in stage.arguments:
                _unary_sites_expression(argument, sites)
...
def _binary_sites_statement(statement, sites: list[tuple[str, int, int]], *, descend_declarations: bool = False) -> None:
...
def _unary_sites_statement(statement, sites: list[tuple[str, int, int]], *, descend_declarations: bool = False) -> None:
    if isinstance(statement, Assignment):
        _unary_sites_expression(statement.expression, sites)
    elif isinstance(statement, Binding):
        _unary_sites_expression(statement.source, sites)
    elif isinstance(statement, Write):
        _unary_sites_expression(statement.expression, sites)
    elif isinstance(statement, ExpressionStatement):
        _unary_sites_expression(statement.expression, sites)
    elif isinstance(statement, IfStatement):
        _unary_sites_expression(statement.condition, sites)
        for item in statement.body:
            _unary_sites_statement(item, sites, descend_declarations=descend_declarations)
        for item in statement.else_body:
            _unary_sites_statement(item, sites, descend_declarations=descend_declarations)
    elif isinstance(statement, ForEach):
        _unary_sites_expression(statement.iterable, sites)
        for item in statement.body:
            _unary_sites_statement(item, sites, descend_declarations=descend_declarations)
    elif isinstance(statement, MatchStatement):
        _unary_sites_expression(statement.subject, sites)
        for case in statement.cases:
            _unary_sites_expression(case.pattern, sites)
            _unary_sites_statement(case.statement, sites, descend_declarations=descend_declarations)
    elif isinstance(statement, TryStatement):
        for item in statement.body:
            _unary_sites_statement(item, sites, descend_declarations=descend_declarations)
        for item in statement.except_body:
        ...
        if statement.inline_expression is not None:
            _unary_sites_expression(statement.inline_expression, sites)
        for item in statement.body:
            _unary_sites_statement(item, sites, descend_declarations=True)


def _call_sites_statement(statement, sites: list[tuple[str, int, int]], *, descend_declarations: bool = False) -> None:
    if isinstance(statement, Assignment):
...
def _match_sites(sites: list[tuple[str, int, int]], instructions, *, flow_name: str | None) -> list[SourceProvenance]:
...
def _match_unary_sites(sites: list[tuple[str, int, int]], instructions, *, flow_name: str | None) -> list[SourceProvenance]:
    result: list[SourceProvenance] = []
    search_from = 0
    for operator, line, column in sites:
        matched_index = None
        for index in range(search_from, len(instructions)):
            instruction = instructions[index]
            if instruction.opcode == "unary" and len(instruction.operands) == 2 and instruction.operands[0] == operator:
                matched_index = index
                break
        if matched_index is None:
            scope = flow_name or "<ana>"
            raise SourceProvenanceError(
                f"Kaynak unary provenance IR ile eşleştirilemedi: {operator!r} satır {line}, sütun {column}, kapsam {scope}."
            )
        result.append(SourceProvenance(matched_index, line, column, "unary", flow_name))
        search_from = matched_index + 1
    return result


def _match_call_sites(sites: list[tuple[str, int, int]], instructions, *, flow_name: str | None) -> list[SourceProvenance]:
    result: list[SourceProvenance] = []
    search_from = 0
...
def _flow_by_name(program: IRProgram, source_name: str) -> IRFlow:
    ...
    if len(matches) != 1:
        raise SourceProvenanceError(f"Kaynak akış provenance için tek bir IRFlow bekleniyordu: {source_name!r}.")
    return matches[0]
...
def build_unary_source_provenance(source: str, program: IRProgram) -> tuple[SourceProvenance, ...]:
    ast: Program = parse(tokenize(source))
    top_sites: list[tuple[str, int, int]] = []
    for statement in ast.statements:
        if not isinstance(statement, Declaration):
            _unary_sites_statement(statement, top_sites)
    result = _match_unary_sites(top_sites, program.instructions, flow_name=None)
    for statement in ast.statements:
        if not isinstance(statement, Declaration) or statement.kind != "akış" or not statement.name:
            continue
        flow_sites: list[tuple[str, int, int]] = []
        if statement.inline_expression is not None:
            _unary_sites_expression(statement.inline_expression, flow_sites)
        for item in statement.body:
            _unary_sites_statement(item, flow_sites, descend_declarations=True)
        flow = _flow_by_name(program, statement.name)
        result.extend(_match_unary_sites(flow_sites, flow.instructions, flow_name=flow.name))
    return tuple(result)
...
def build_source_provenance(source: str, program: IRProgram) -> tuple[SourceProvenance, ...]:
    """Hata payload ABI için binary, unary ve call-site provenance kanıtını üretir."""
    return (
        *build_binary_source_provenance(source, program),
        *build_unary_source_provenance(source, program),
        *build_call_source_provenance(source, program),
    )